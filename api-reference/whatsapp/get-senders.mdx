...
```bash cURL
curl -X GET "https://repforce.ai/api/user/whatsapp/senders" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

```bash Filter all senders
curl -X GET "https://repforce.ai/api/user/whatsapp/senders?status=all" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

```javascript JavaScript
const response = await fetch(
  'https://repforce.ai/api/user/whatsapp/senders',
  {
    headers: {
